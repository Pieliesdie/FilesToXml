...
public class DbfToXml : IConvertable
{
    ...
    public static IEnumerable<XStreamingElement> ReadLines(Stream stream)
    {
        ...
        var lineNumber = 0;
        IEnumerable<XAttribute> headerRow = dbfTable.Columns.Select((column, index) => new XAttribute($"C{index + 1}", column.ColumnName));
        yield return new XStreamingElement("R", new XAttribute("id", ++lineNumber), headerRow);
        while (dbfTable.Read(dbfRecord))
        {
            IEnumerable<XAttribute> attrs = dbfRecord.Values.Select((value, index) => new XAttribute($"C{index + 1}", value));
