...
public class ShapeOptions : Record
{
    ...
    public Dictionary<PropertyId, OptionEntry> OptionsByID = new();
    
    public ShapeOptions()
    {
        Options = new OptionEntry[0];
    }
        ...
        : base(_reader, size, typeCode, version, instance)
    {
        ...
        Options = new OptionEntry[instance];
        ...
        for (var i = 0; i < instance; i++)
        {
            ...
            {
                tempPos = Reader.BaseStream.Position;
                if (Options[i].op > 0)
                {
                    Options[i].opComplex = Reader.ReadBytes((int)Options[i].op);
                }
                
                if (Options[i].pid == PropertyId.pVertices)
                {
                    var nElemsVert = BitConverter.ToUInt16(Options[i].opComplex, 0);
                    var nElemsAllocVert = BitConverter.ToUInt16(Options[i].opComplex, 2);
                    var cbElemVert = BitConverter.ToUInt16(Options[i].opComplex, 4);
                    if (cbElemVert == 0xfff0)
                    {
                        cbElemVert = 4;
                    }
                    
                    if (nElemsVert * cbElemVert == Options[i].op)
                    {
                        Reader.BaseStream.Seek(tempPos, SeekOrigin.Begin);
                        Options[i].opComplex = Reader.ReadBytes((int)Options[i].op + 6);
                    }
            ...
            if (OptionsByID.ContainsKey(Options[i].pid))
            {
                OptionsByID[Options[i].pid] = Options[i];
            }
            else
            {
                OptionsByID.Add(Options[i].pid, Options[i]);
            }
        }
