...
public class XmlContainer : XmlRecord
{
        ...
        : base(_reader, size, typeCode, version, instance)
    {
        ...
        using (var zipReader = ZipFactory.OpenArchive(Reader.BaseStream))
        {
            XmlDocumentElement = ExtractDocumentElement(zipReader, GetRelations(zipReader, ""));
        }
    }
    ...
    {
        var relPath = GetRelationPath(forPartPath);
    ...
    protected static string GetRelationPath(string forPartPath)
    {
        ...
        var relPath = $"{directoryPath}_rels/{filePath}.rels";
        return relPath;
    }
    ...
    protected virtual XmlElement ExtractDocumentElement(IZipReader zipReader, XmlNodeList rels)
    {
        ...
        var partPath = rels[0].Attributes["Target"].Value;
