...
public class BRAI : OfficeGraphBiffRecord
{
    ...
    /// <summary>
    ///     A bit that specifies whether the part of the chart specified by the id field uses number formatting from the
    ///     referenced data.
    /// </summary>
        ...
        : base(reader, id, length)
    {
        ...
        braiId = (BraiId)reader.ReadByte();
        rt = reader.ReadByte();
        fUnlinkedIfmt = (Formatting)Utils.BitmaskToUInt16(reader.ReadUInt16(), 0x1);
        ifmt = reader.ReadUInt16();
        rowcol = reader.ReadUInt16();
