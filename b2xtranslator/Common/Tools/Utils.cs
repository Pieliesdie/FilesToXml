...
public class Utils
{
    ...
    {
        var s = replaceOutdatedNamespaces(e.OuterXml);
    ...
    {
        var output = input.Replace("http://schemas.openxmlformats.org/drawingml/2006/3/main", "http://schemas.openxmlformats.org/drawingml/2006/main");
    ...
    public static string ReadLengthPrefixedUnicodeString(Stream stream)
    {
        ...
        var cch = BitConverter.ToInt32(cchBytes, 0);
    ...
    {
        var xstz = new byte[BitConverter.ToUInt16(bytes, pos) * 2];
    ...
    public static string ReadXst(Stream stream)
    {
        ...
        var cch = new byte[2];
        stream.Read(cch, 0, cch.Length);
        var charCount = BitConverter.ToUInt16(cch, 0);
    ...
    public static BitArray BitArrayCopy(BitArray source, int sourceIndex, int copyCount)
    {
        ...
        var j = 0;
        for (var i = sourceIndex; i < copyCount + sourceIndex; i++)
        {
            ret[j] = source[i];
            j++;
        }
