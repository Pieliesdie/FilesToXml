...
public class SinglePropertyModifier : IVisitable
{
    ...
    public SinglePropertyModifier(byte[] bytes)
    {
        ...
        OpCode = (OperationCode)BitConverter.ToUInt16(bytes, 0);
        
        //... whereof bit 9 is fSpec ...
        var j = (uint)OpCode << 22;
        j = j >> 31;
        if (j == 1)
        {
            fSpec = true;
        ...
        var i = (uint)OpCode << 19;
        i = i >> 29;
        Type = (SprmType)i;
        
        //... and last 3 bits are the spra
        var spra = (byte)((int)OpCode >> 13);
        var opSize = GetOperandSize(spra);
        if (opSize == 255)
        {
            switch (OpCode)
            {
                ...
                case OperationCode.sprmTDefTable10:
                    ...
                    var opSizeTable = BitConverter.ToInt16(bytes, 2);
                    //and the arguments start at the byte after that (byte3)
                    Arguments = new byte[opSizeTable - 1];
                    //Arguments start at byte 4
                    Array.Copy(bytes, 4, Arguments, 0, Arguments.Length);
                    break;
                case OperationCode.sprmPChgTabs:
                    Arguments = new byte[bytes[2]];
                    Array.Copy(bytes, 3, Arguments, 0, Arguments.Length);
                    break;
                default:
                    //the variable length stand in the byte after the opcode (byte2)
                    opSize = bytes[2];
                    //and the arguments start at the byte after that (byte3)
                    Arguments = new byte[opSize];
                    Array.Copy(bytes, 3, Arguments, 0, Arguments.Length);
                    break;
            }
        }
        ...
        {
            Arguments = new byte[opSize];
            Array.Copy(bytes, 2, Arguments, 0, Arguments.Length);
        }
    }
