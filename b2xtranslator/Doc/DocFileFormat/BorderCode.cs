...
public class BorderCode : IVisitable
{
    ...
    public BorderCode(byte[] bytes)
    {
        ...
        else if (bytes.Length == 8)
        {
            ...
            cv = BitConverter.ToInt32(bytes, 0);
            ico = Global.ColorIdentifier.auto;
            
            dptLineWidth = bytes[4];
            brcType = bytes[5];
            
            var val = BitConverter.ToInt16(bytes, 6);
            dptSpace = val & 0x001F;
            
            //not sure if this is correct, the values from the spec are definitly wrong:
            fShadow = Utils.BitmaskToBool(val, 0x20);
            fFrame = Utils.BitmaskToBool(val, 0x40);
        }
        else if (bytes.Length == 4)
        {
            //it's a border code of Word 97
            var val = BitConverter.ToUInt16(bytes, 0);
            dptLineWidth = (byte)(val & 0x00FF);
            brcType = (byte)((val & 0xFF00) >> 8);
            val = BitConverter.ToUInt16(bytes, 2);
            ico = (Global.ColorIdentifier)(val & 0x00FF);
