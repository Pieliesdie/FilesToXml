...
public class SprmTDefTable
{
    ...
    public SprmTDefTable(byte[] bytes)
    {
        ...
        //read rgdxaCenter
        rgdxaCenter = new short[numberOfColumns + 1];
        for (var i = 0; i < numberOfColumns + 1; i++)
        {
            rgdxaCenter[i] = BitConverter.ToInt16(bytes, pointer);
            pointer += 2;
        }
        
        //read rgTc80
        rgTc80 = new TC80[numberOfColumns];
        for (var i = 0; i < numberOfColumns; i++)
        {
            ...
            {
                //the flags
                var flags = BitConverter.ToUInt16(bytes, pointer);
                tc.horzMerge = (byte)Utils.BitmaskToInt(flags, 0x3);
                tc.textFlow = (Global.TextFlow)Utils.BitmaskToInt(flags, 0x1C);
                tc.vertMerge = (Global.VerticalMergeFlag)Utils.BitmaskToInt(flags, 0x60);
                tc.vertAlign = (Global.VerticalAlign)Utils.BitmaskToInt(flags, 0x180);
                tc.ftsWidth = (Global.CellWidthType)Utils.BitmaskToInt(flags, 0xE00);
                ...
                //cell width
                tc.wWidth = BitConverter.ToInt16(bytes, pointer);
                pointer += 2;
...
{
    /// <summary>
    ///     A value from the following table that specifies how this cell merges horizontally with the neighboring cells in the
    ///     same row. <br />
    ///     MUST be one of the following values:<br />
    ///     0        The cell is not merged with the cells on either side of it.
    ///     1        The cell is one of a set of horizontally merged cells. It contributes its layout region to the set and its
    ///     own contents are not rendered.
    ///     2, 3     The cell is the first cell in a set of horizontally merged cells. The contents and formatting of this cell
    ///     extend into any consecutive cells following it that are designated as part of the merged set.
    /// </summary>
    ...
    /// <summary>
    ///     A value from the VerticalMergeFlag enumeration that specifies how this cell merges vertically with the cells above
    ///     or below it.
    /// </summary>
