    ...
    public PropertyExceptions(byte[] bytes)
    {
        ...
        if (bytes.Length != 0)
        {
            ...
            while (goOn)
            {
                ...
                {
                    //make spra
                    var opCode = (SinglePropertyModifier.OperationCode)BitConverter.ToUInt16(bytes, sprmStart);
                    var spra = (byte)((int)opCode >> 13);
                    
                    // get size of operand
                    short opSize = SinglePropertyModifier.GetOperandSize(spra);
                    byte lenByte = 0;
                    ...
                    if (opSize == 255)
                    {
                        ...
                        switch (opCode)
                        {
                            ...
                            case SinglePropertyModifier.OperationCode.sprmTDefTable10:
                                ...
                                lenByte = 2;
                                opSize = BitConverter.ToInt16(bytes, sprmStart + 2);
                            ...
                            case SinglePropertyModifier.OperationCode.sprmPChgTabs:
                                ...
                                if (opSize == 255)
                                {
                                    var itbdDelMax = bytes[sprmStart + 3];
                                    var itbdAddMax = bytes[sprmStart + 3 + 2 * itbdDelMax];
