    ...
    IMapping<TablePropertyExceptions>
{
    ...
    private readonly XmlElement _tcBorders;
    private TC80 _tcDef;
    private readonly XmlElement _tcMar;
    private readonly XmlElement _tcPr;
    ...
    public void Apply(TablePropertyExceptions tapx)
    {
        ...
        for (var i = 0; i < tapx.grpprl.Count; i++)
        {
            ...
            switch (sprm.OpCode)
            {
                ...
                case SinglePropertyModifier.OperationCode.sprmTCellPadding:
                    var first = sprm.Arguments[0];
                    var lim = sprm.Arguments[1];
                    var ftsMargin = sprm.Arguments[3];
                    var wMargin = BitConverter.ToInt16(sprm.Arguments, 4);
                    
                    if (_cellIndex >= first && _cellIndex < lim)
                    {
                        var borderBits = new BitArray(new[] { sprm.Arguments[2] });
                        if (borderBits[0])
                        {
                ...
                case SinglePropertyModifier.OperationCode.sprmTSetBrc:
                    var min = sprm.Arguments[0];
                    var max = sprm.Arguments[1];
        ...
        if (_width > _grid[_gridIndex])
        {
            ...
            for (var i = _gridIndex + 1; i < _grid.Count; i++)
            {
                GridSpan++;
                w += _grid[i];
                if (w >= _width)
            ...
            appendValueElement(_tcPr, "gridSpan", GridSpan.ToString(), true);
        }
