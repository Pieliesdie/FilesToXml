    ...
    IMapping<ListTable>
{
    ...
    public void Apply(ListTable rglst)
    {
        ...
        for (var i = 0; i < rglst.Count; i++)
        {
            ...
            _writer.WriteAttributeString("w", "val", OpenXmlNamespaces.WordprocessingML, $"{lstf.lsid:X8}");
            ...
            _writer.WriteAttributeString("w", "val", OpenXmlNamespaces.WordprocessingML, $"{lstf.tplc:X8}");
            _writer.WriteEndElement();
        ...
        //write the overrides
        for (var i = 0; i < _ctx.Doc.ListFormatOverrideTable.Count; i++)
        {
            ...
            var index = FindIndexbyId(rglst, lfo.lsid);
