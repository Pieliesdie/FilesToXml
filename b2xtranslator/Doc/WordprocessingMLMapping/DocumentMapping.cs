    ...
    IMapping<WordDocument>
{
    ...
    protected List<short> buildTableGrid(int initialCp, uint nestingLevel)
    {
        ...
        while (tai.fInTable)
        {
            ...
            foreach (var sprm in papx.grpprl)
            {
                ...
                {
                    var itcMac = sprm.Arguments[0];
                    for (var i = 0; i < itcMac; i++)
                    {
                        var boundary1 = BitConverter.ToInt16(sprm.Arguments, 1 + i * 2);
                        if (!boundaries.Contains(boundary1))
                        {
                            boundaries.Add(boundary1);
                        }
                        
                        var boundary2 = BitConverter.ToInt16(sprm.Arguments, 1 + (i + 1) * 2);
    ...
    protected int findRowEndFc(int initialCp, out int rowEndCp, uint nestingLevel)
    {
        ...
        if (nestingLevel > 1)
        {
            ...
            while (tai.fInnerTtp == false && tai.fInTable)
            {
        ...
        else
        {
            ...
            while (tai.fTtp == false && tai.fInTable)
            {
    ...
    {
        var cpCellEnd = initialCp;
        
        if (nestingLevel > 1)
        {
            var fc = _doc.PieceTable.FileCharacterPositions[initialCp];
            var papx = findValidPapx(fc);
            var tai = new TableInfo(papx);
    ...
    protected int writeParagraph(int cp)
    {
        ...
        if (_doc.Text[cpParaEnd] == TextMark.PageBreakOrSectionMark)
        {
            ...
            var sectionEnd = isSectionEnd(cpParaEnd);
            cpParaEnd++;
    ...
    protected int writeParagraph(int initialCp, int cpEnd, bool sectionEnd)
    {
        var cp = initialCp;
        var fc = _doc.PieceTable.FileCharacterPositions[cp];
        var fcEnd = _doc.PieceTable.FileCharacterPositions[cpEnd];
        var papx = findValidPapx(fc);
        ...
        //start paragraph
        _writer.WriteStartElement("w", "p", OpenXmlNamespaces.WordprocessingML);
        _writer.WriteAttributeString("w14", "paraId", OpenXmlNamespaces.WordprocessingML2010, Guid.NewGuid().ToString());
        _writer.WriteAttributeString("w14", "textId", OpenXmlNamespaces.WordprocessingML2010, "77777777");
        ...
        for (var i = 0; i < chpxs.Count; i++)
        {
            //get the FC range for this run
            var fcChpxStart = chpxFcs[i];
            var fcChpxEnd = chpxFcs[i + 1];
            ...
            if (bookmarks.Count > 0)
            {
                ...
                for (var s = 0; s < runs.Count; s++)
                {
                        ...
                        _doc.BookmarkEndPlex.CharacterPositions.Contains(cp))
                    {
                        ...
                        for (var b = 0; b < _doc.BookmarkEndPlex.CharacterPositions.Count; b++)
                        {
    ...
    protected int writeRun(List<char> chars, CharacterPropertyExceptions chpx, int initialCp)
    {
        ...
        if (_skipRuns <= 0 && chars.Count > 0)
        {
            ...
            if (rev.RsidProp != 0)
            {
                var rsidProp = $"{rev.RsidProp:x8}";
                _writer.WriteAttributeString("w", "rsidP", OpenXmlNamespaces.WordprocessingML, rsidProp);
    ...
    protected void writeText(List<char> chars, int initialCp, CharacterPropertyExceptions chpx, bool writeDeletedText)
    {
        ...
        for (var i = 0; i < chars.Count; i++)
        {
            ...
            else if (c == TextMark.FieldBeginMark)
            {
                ...
                var cpFieldStart = initialCp + i;
                var cpFieldEnd = searchNextTextMark(_doc.Text, cpFieldStart, TextMark.FieldEndMark);
                ...
                if (f.FieldCode.StartsWith(" FORM"))
                {
                    ...
                    var cpPic = searchNextTextMark(_doc.Text, cpFieldStart, TextMark.Picture);
                    if (cpPic < cpFieldEnd)
                    {
                        var fcPic = _doc.PieceTable.FileCharacterPositions[cpPic];
                ...
                else if (f.FieldCode.StartsWith(" EMBED") || f.FieldCode.StartsWith(" LINK"))
                {
                    ...
                    var cpPic = searchNextTextMark(_doc.Text, cpFieldStart, TextMark.Picture);
                    var cpFieldSep = searchNextTextMark(_doc.Text, cpFieldStart, TextMark.FieldSeperator);
                    ...
                    if (cpPic < cpFieldEnd)
                    {
                        ...
                        pic.Convert(new VMLPictureMapping(_writer, _targetPart, true));
                        
                        if (cpFieldSep < cpFieldEnd)
                        {
                            var fcFieldSep = _doc.PieceTable.FileCharacterPositions[cpFieldSep];
            ...
            else if (c == TextMark.DrawnObject && fSpec)
            {
                ...
                {
                    var headerCp = cp - _doc.FIB.ccpText - _doc.FIB.ccpFtn;
    ...
    protected void writeBookmarkStarts(int cp)
    {
        ...
        {
            for (var b = 0; b < _doc.BookmarkStartPlex.CharacterPositions.Count; b++)
            {
                if (_doc.BookmarkStartPlex.CharacterPositions[b] == cp)
                {
                    if (_doc.BookmarkStartPlex.Elements.Count > b)
    ...
    protected List<List<char>> splitCharList(List<char> chars, List<int> splitIndices)
    {
        ...
        //add the parts
        for (var i = 0; i < splitIndices.Count; i++)
        {
            var cch = splitIndices[i] - startIndex;
            if (cch > 0)
            {
    ...
    protected bool isSpecial(CharacterPropertyExceptions chpx)
    {
        ...
        foreach (var sprm in chpx.grpprl)
        {
            ...
            if (sprm.OpCode == SinglePropertyModifier.OperationCode.sprmCFSpec)
            {
                //special value
    ...
    private Symbol getSymbol(CharacterPropertyExceptions chpx)
    {
        ...
        foreach (var sprm in chpx.grpprl)
        {
            if (sprm.OpCode == SinglePropertyModifier.OperationCode.sprmCSymbol)
            {
                //special symbol
                ret = new Symbol();
                var fontIndex = BitConverter.ToInt16(sprm.Arguments, 0);
                var code = BitConverter.ToInt16(sprm.Arguments, 2);
    ...
    protected bool isSectionEnd(int cp)
    {
        ...
        for (var i = 0; i < _doc.SectionPlex.CharacterPositions.Count; i++)
        {
    ...
    protected SectionPropertyExceptions findValidSepx(int cp)
    {
        ...
        catch (KeyNotFoundException)
        {
            ...
            var lastKey = _doc.SectionPlex.CharacterPositions[1];
            foreach (var key in _doc.AllSepx.Keys)
            {
