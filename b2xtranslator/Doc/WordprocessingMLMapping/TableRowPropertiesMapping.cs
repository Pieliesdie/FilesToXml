...
internal class TableRowPropertiesMapping :
    PropertiesMapping,
    IMapping<TablePropertyExceptions>
{
    ...
    private readonly CharacterPropertyExceptions _rowEndChpx;
    private readonly XmlElement _tblPrEx;
    private readonly XmlElement _trPr;
        ...
        foreach (var sprm in tapx.grpprl)
        {
            switch (sprm.OpCode)
            {
                ...
                case SinglePropertyModifier.OperationCode.sprmTTableHeader:
                    var fHeader = Utils.ByteToBool(sprm.Arguments[0]);
                    if (fHeader)
                    {
                ...
                case SinglePropertyModifier.OperationCode.sprmTWidthBefore:
                    var before = BitConverter.ToInt16(sprm.Arguments, 1);
                    if (before != 0)
                    {
                ...
                case SinglePropertyModifier.OperationCode.sprmTDyaRowHeight:
                    ...
                    var rH = BitConverter.ToInt16(sprm.Arguments, 0);
                    if (rH > 0)
                    {
